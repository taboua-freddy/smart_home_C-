...
bool VehiculeProprietaire::ajouterVehicule()
{
    int n=0;
    QSqlQuery query;
    query.prepare("select matricule from vehicule_proprietaire where matricule=:mat");
...
void VehiculeProprietaire::recupererInfo(int num, QString &marque, int &id,QString &etat)
{
    QSqlQuery query;
    query.prepare("select v.marque,v.id,p.etat,v.numero_place from vehicule_proprietaire v left outer join place_parking p on v.numero_place=p.numero_place where v.numero_place=:num ");
    query.bindValue(":num",num);
    query.exec();
    while (query.next()) {
        marque=query.value(0).toString();
        id=query.value(1).toInt();
        etat=query.value(2).toString();
    }
}
